...
namespace Multiple_concurrencia
{
    class pgConnect
    {
        ...
        public pgConnect()
        {
            var conString = String.Format("Host={0};Username={1};Password={2};Database={3};Pooling = false", host, username, password, db_name);
        ...
        public void do_select()
        {
            ...
            {
                if (ex.Message.Equals("Connection is not open"))
                {
                ...
                else if (ex.Message.Equals("There is not enough space left in the buffer"))
                {
                    Thread.Sleep(100);
                    do_select();
                }
